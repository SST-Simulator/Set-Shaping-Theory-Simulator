...
function adaptiveArithmeticKtLength(sequence) {
  const symbolCount = Math.max(...sequence);
  const counts = Array(symbolCount).fill(0);
  const alpha = 0.5;
  let lengthBits = 0;
...
function enumerativeCodeLength(sequence) {
...
function lz78CodeLength(sequence) {
  const symbolCount = Math.max(...sequence);
...
function adaptiveHuffmanLength(sequence) {
  const symbolCount = Math.max(...sequence);
  const counts = Array(symbolCount).fill(1);
  let lengthBits = 0;
...
function adaptiveAnsLength(sequence) {
  const symbolCount = Math.max(...sequence);
  const counts = Array(symbolCount).fill(0);
