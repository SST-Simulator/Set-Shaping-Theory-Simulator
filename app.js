...
const universalForm = document.querySelector("#universal-form");
const universalSamplesHInput = document.querySelector("#universal-samples-h");
const universalLengthNInput = document.querySelector("#universal-length-n");
const universalAlphabetInput = document.querySelector("#universal-alphabet");
const universalLengthKInput = document.querySelector("#universal-length-k");
const universalMaxWorkInput = document.querySelector("#universal-max-work");
const universalSummary = document.querySelector("#universal-summary");
const universalNote = document.querySelector("#universal-note");
const universalMetrics = document.querySelector("#universal-metrics");
const universalBody = document.querySelector("#universal-body");
const universalProgress = document.querySelector("#universal-progress");
...
const DEFAULT_VIEW_ID = "home-view";
...
navigationButtons.forEach((button) => {
  button.addEventListener("click", () => {
    navigateToView(button.dataset.viewTarget, {
      replace: button.classList.contains("back-link") && button.dataset.viewTarget === DEFAULT_VIEW_ID,
    });
  });
});
...
if (universalForm) {
  universalForm.addEventListener("submit", async (event) => {
    event.preventDefault();
    await runWithProgress(universalProgress, calculateUniversalCoders);
  });
}
...
function showView(targetId) {
  const safeTargetId = getValidViewId(targetId);

  views.forEach((view) => {
    view.hidden = view.id !== safeTargetId;
  });

  const targetView = document.querySelector(`#${safeTargetId}`);
  typesetMath(targetView);
}

function navigateToView(targetId, options = {}) {
  const safeTargetId = getValidViewId(targetId);
  showView(safeTargetId);

  if (options.replace) {
    window.history.replaceState({ viewId: safeTargetId }, "", urlForView(safeTargetId));
    return;
  }

  window.history.pushState({ viewId: safeTargetId }, "", urlForView(safeTargetId));
}

function getValidViewId(targetId) {
  const view = targetId ? document.getElementById(targetId) : null;
  return view && view.classList.contains("view") ? targetId : DEFAULT_VIEW_ID;
}

function viewIdFromLocation() {
  return getValidViewId(window.location.hash.replace("#", ""));
}

function urlForView(targetId) {
  if (targetId === DEFAULT_VIEW_ID) {
...
window.addEventListener("popstate", (event) => {
  showView(event.state?.viewId || viewIdFromLocation());
});
...
async function calculateUniversalCoders(progress = null) {
  try {
    setUniversalStatus("Computing...");

    const h = parseInteger(universalSamplesHInput.value, "H", 1);
    const n = parseInteger(universalLengthNInput.value, "N", 2);
    const alphabetSize = parseInteger(universalAlphabetInput.value, "A", 10);
    const k = parseInteger(universalLengthKInput.value, "K", 0);
    const maxWork = parseInteger(universalMaxWorkInput.value, "computation limit", 1);
    const transformsPerSequence = power(alphabetSize, k);
    const work = h * n * transformsPerSequence;

    if (alphabetSize > 1024) {
      throw new Error("A must be less than or equal to 1024 for the adaptive ANS model.");
    }

    if (!Number.isSafeInteger(work) || work > maxWork) {
      throw new Error(`This run needs about ${formatNumber(work)} operations: increase the limit or reduce H, N, A, or K.`);
    }

    const totals = createUniversalTotals();
    const yieldEvery = Math.max(1, Math.floor(h / 120));

    for (let sampleIndex = 0; sampleIndex < h; sampleIndex += 1) {
      const sequence = randomMatlabSequence(alphabetSize, n);
      const transformed = transformUniversalSst(sequence, k, alphabetSize);
      const originalEntropyScore = universalEntropyScore(sequence, alphabetSize);
      const transformedEntropyScore = universalEntropyScore(transformed, alphabetSize);
      const originalKt = ktExactFromSequence(sequence, alphabetSize);
      const transformedKt = ktExactFromSequence(transformed, alphabetSize);
      const originalArithmetic = adaptiveArithmeticKtLength(sequence, alphabetSize);
      const transformedArithmetic = adaptiveArithmeticKtLength(transformed, alphabetSize);
      const originalEnumerative = enumerativeCodeLength(sequence, alphabetSize);
      const transformedEnumerative = enumerativeCodeLength(transformed, alphabetSize);
      const originalLz78 = lz78CodeLength(sequence, alphabetSize);
      const transformedLz78 = lz78CodeLength(transformed, alphabetSize);
      const originalHuffman = adaptiveHuffmanLength(sequence, alphabetSize);
      const transformedHuffman = adaptiveHuffmanLength(transformed, alphabetSize);
      const originalAns = adaptiveAnsLength(sequence, alphabetSize);
      const transformedAns = adaptiveAnsLength(transformed, alphabetSize);

      addUniversalResult(totals, "entropy", originalEntropyScore, transformedEntropyScore);
      addUniversalResult(totals, "ktRedundancy", originalKt.redundancy, transformedKt.redundancy);
      addUniversalResult(totals, "ktExact", originalKt.length, transformedKt.length);
      addUniversalResult(totals, "arithmetic", originalArithmetic, transformedArithmetic);
      addUniversalResult(totals, "ans", originalAns, transformedAns);
      addUniversalResult(totals, "enumerative", originalEnumerative, transformedEnumerative);
      addUniversalResult(totals, "lz78", originalLz78, transformedLz78);
      addUniversalResult(totals, "huffman", originalHuffman, transformedHuffman);
      totals.transformedLength += transformed.length;

      if (sampleIndex % yieldEvery === 0) {
        progress?.update((sampleIndex + 1) / h, "Universal coders");
        await nextFrame();
      }
    }

    const rows = universalRowsFromTotals(totals, h);
    const rankedRows = rows.filter((row) => !row.isDiagnostic);
    const bestRow = rankedRows.reduce((best, row) => (
      row.gainBits > best.gainBits ? row : best
    ), rankedRows[0]);

    renderUniversalSummary({
      h,
      transformsPerSequence,
      transformedLength: totals.transformedLength / h,
      state: "Completed",
      isError: false,
    });
    renderUniversalMetrics({
      bestGainBits: bestRow.gainBits,
      bestGainPercent: bestRow.gainPercent,
      bestCoder: bestRow.coder,
    });
    renderUniversalRows(rows);
    universalNote.textContent = `Processed ${formatNumber(h)} random sequences with N=${formatNumber(n)}, A=${formatNumber(alphabetSize)}, and K=${formatNumber(k)}. Lengths are measured in bits.`;
  } catch (error) {
    renderUniversalSummary({
      h: "-",
      transformsPerSequence: "-",
    ...
    renderUniversalMetrics(null);
    renderUniversalEmpty(error.message, true);
    universalNote.textContent = "Fix the parameters and try again.";
  }
}
...
async function calculateRandomGraphMethod(progress = null) {
  try {
    ...
    if (ar + 1 > possibleExpandedEdges) {
      throw new Error(`For Nd+1=${expandedNodes}, the transformed graph cannot contain Ar+1 edges.`);
    }
...
function randomMatlabSequence(symbolCount, length) {
  return Array.from({ length }, () => Math.floor(Math.random() * symbolCount) + 1);
}

function createUniversalTotals() {
  return {
    transformedLength: 0,
    entropy: createUniversalCoderTotal(),
    ktRedundancy: createUniversalCoderTotal(),
    ktExact: createUniversalCoderTotal(),
    arithmetic: createUniversalCoderTotal(),
    ans: createUniversalCoderTotal(),
    enumerative: createUniversalCoderTotal(),
    lz78: createUniversalCoderTotal(),
    huffman: createUniversalCoderTotal(),
  };
}

function createUniversalCoderTotal() {
  return {
    original: 0,
    transformed: 0,
    improved: 0,
...
function addUniversalResult(totals, key, originalLength, transformedLength) {
  totals[key].original += originalLength;
  totals[key].transformed += transformedLength;

  if (originalLength > transformedLength) {
    totals[key].improved += 1;
  }
}

function universalRowsFromTotals(totals, sampleCount) {
  return [
    universalRowFromTotal("Empirical entropy N*H0", totals.entropy, sampleCount),
    universalRowFromTotal("KT redundancy R_KT", totals.ktRedundancy, sampleCount, { isDiagnostic: true }),
    universalRowFromTotal("N*H0(s) + R_KT = L_KT", totals.ktExact, sampleCount),
    universalRowFromTotal("Adaptive arithmetic", totals.arithmetic, sampleCount),
    universalRowFromTotal("Adaptive ANS", totals.ans, sampleCount),
    universalRowFromTotal("Enumerative coding", totals.enumerative, sampleCount),
    universalRowFromTotal("LZ78", totals.lz78, sampleCount),
    universalRowFromTotal("Adaptive Huffman", totals.huffman, sampleCount),
  ];
}

function universalRowFromTotal(coder, total, sampleCount, options = {}) {
  const originalMean = total.original / sampleCount;
  const sstMean = total.transformed / sampleCount;
  const gainBits = originalMean - sstMean;
  const gainPercent = originalMean === 0 ? 0 : (gainBits / originalMean) * 100;
  const improvedPercent = (total.improved / sampleCount) * 100;
  ...
  return {
    ...
    gainPercent,
    improvedPercent,
    isDiagnostic: Boolean(options.isDiagnostic),
  };
}

function transformUniversalSst(sequence, k, symbolCount = Math.max(...sequence)) {
  const length = sequence.length;
  const sourceInformation = universalEntropyScore(sequence, symbolCount);
  const uniformInformation = length * Math.log2(symbolCount);

  if (sourceInformation <= uniformInformation / 2) {
    return transformUniversalLowInformation(sequence, symbolCount, k);
  }

  let bestSequence = null;
  let bestInformation = Number.POSITIVE_INFINITY;
  let fallbackSequence = null;
  let fallbackInformation = Number.POSITIVE_INFINITY;
  const transformsPerSequence = power(symbolCount, k);

  for (let transformIndex = 0; transformIndex < transformsPerSequence; transformIndex += 1) {
    const transformed = universalModularCandidate(sequence, symbolCount, transformIndex, k);
    const information = universalEntropyScore(transformed, symbolCount);
    const hasAllSymbols = Math.max(...transformed) === symbolCount;

    if (information < fallbackInformation) {
      fallbackInformation = information;
      fallbackSequence = transformed;
    }

    if (hasAllSymbols && information < bestInformation) {
      bestInformation = information;
      bestSequence = transformed;
    }
  }

  return bestSequence || fallbackSequence || [
    ...sequence,
    ...universalMetadataSymbols(0, k, symbolCount),
  ];
}

function universalModularCandidate(sequence, symbolCount, transformIndex, k) {
  let state = 3;
  const seed = transformIndex + 1;
  const transformed = sequence.map((symbol) => {
    state = (33 * state + seed) % 10000000;
    const modifier = Math.max(1, Math.round(state / 10000));
    return ((symbol + modifier) % symbolCount) + 1;
  });
  transformed.push(...universalMetadataSymbols(transformIndex, k, symbolCount));
  return transformed;
}

function universalMetadataSymbols(transformIndex, k, symbolCount) {
  const metadata = Array(k).fill(1);
  let value = transformIndex;

  for (let index = k - 1; index >= 0; index -= 1) {
    metadata[index] = (value % symbolCount) + 1;
...
function transformUniversalLowInformation(sequence, symbolCount, k) {
  const transformed = [...sequence];
  const halfLength = Math.round(sequence.length / 2);

  for (let index = 1; index <= halfLength; index += 1) {
    const right = (index * 2) - 1;
    const left = right - 1;

    if (right < sequence.length && sequence[left] + sequence[right] <= symbolCount) {
      transformed[left] = sequence[right];
      transformed[right] = sequence[left];
    }
  }

  transformed.push(...universalMetadataSymbols(0, k, symbolCount));
  return transformed;
}

function universalEntropyScore(sequence, symbolCount = Math.max(...sequence)) {
  const counts = Array(symbolCount).fill(0);
  ...
  return sequence.length * empiricalEntropy(counts, sequence.length);
}

function ktExactFromSequence(sequence, symbolCount) {
  const counts = countsFromMatlabSymbols(sequence, symbolCount);
  const n = sequence.length;
  const entropyScore = n * empiricalEntropy(counts, n);
  const lnProbability = (
    logGamma(symbolCount / 2)
    - ((symbolCount / 2) * Math.log(Math.PI))
    + counts.reduce((sum, count) => sum + logGamma(count + 0.5), 0)
    - logGamma(n + (symbolCount / 2))
  );
  const length = -lnProbability / Math.log(2);

  return {
    length,
    entropyScore,
    redundancy: length - entropyScore,
  };
}

function countsFromMatlabSymbols(sequence, symbolCount) {
  const counts = Array(symbolCount).fill(0);
...
function adaptiveArithmeticKtLength(sequence, symbolCount = Math.max(...sequence)) {
  const counts = Array(symbolCount).fill(0);
  const alpha = 0.5;
  let lengthBits = 0;

  sequence.forEach((symbol, index) => {
    const probability = (counts[symbol - 1] + alpha) / (index + (symbolCount * alpha));
    lengthBits -= Math.log2(probability);
    counts[symbol - 1] += 1;
  });
...
function enumerativeCodeLength(sequence, symbolCount = Math.max(...sequence)) {
  const length = sequence.length;
  const counts = Array(symbolCount).fill(0);
  ...
  const logMultinomial = (
    logGamma(length + 1) - counts.reduce((sum, count) => sum + logGamma(count + 1), 0)
  ) / Math.log(2);
  const typeCost = (symbolCount - 1) * Math.log2(length + 1);
  return typeCost + logMultinomial;
}

function lz78CodeLength(sequence, symbolCount = Math.max(...sequence)) {
  const dictionary = new Map();
  const symbolCost = Math.ceil(Math.log2(symbolCount));
  let nextIndex = 1;
  let position = 0;
  let lengthBits = 0;
  ...
  while (position < sequence.length) {
    ...
    while (scan < sequence.length) {
      const candidate = `${phrase}_${sequence[scan]}`;
    ...
    const pointerCost = Math.ceil(Math.log2(nextIndex + 1));

    if (scan < sequence.length) {
      const newPhrase = `${phrase}_${sequence[scan]}`;
      dictionary.set(newPhrase, nextIndex);
      nextIndex += 1;
      lengthBits += pointerCost + symbolCost;
      position = scan + 1;
    } else {
      lengthBits += pointerCost;
...
function adaptiveHuffmanLength(sequence, symbolCount = Math.max(...sequence)) {
  const counts = Array(symbolCount).fill(0.5);
  let lengthBits = 0;

  sequence.forEach((symbol) => {
    const codeLengths = huffmanLengthsFromCounts(counts);
    lengthBits += codeLengths[symbol - 1];
    counts[symbol - 1] += 1;
  });
...
function huffmanLengthsFromCounts(counts) {
  if (counts.length === 1) {
    return [1];
  }

  let nodes = counts.map((weight, index) => ({
    weight,
    symbols: [index],
  }));
  const codeLengths = Array(counts.length).fill(0);

  while (nodes.length > 1) {
    nodes.sort((left, right) => left.weight - right.weight);
    const first = nodes.shift();
    const second = nodes.shift();

    first.symbols.forEach((symbol) => {
...
function adaptiveAnsLength(sequence, symbolCount = Math.max(...sequence)) {
  const counts = Array(symbolCount).fill(0);
  ...
  sequence.forEach((symbol, index) => {
    const denominator = index + (symbolCount * alpha);
    const probabilities = counts.map((count) => (count + alpha) / denominator);
    const frequencies = quantizeProbabilitiesToAnsFrequencies(probabilities, tableSize);
    lengthBits += Math.log2(tableSize) - Math.log2(frequencies[symbol - 1]);
    counts[symbol - 1] += 1;
  });
...
function quantizeProbabilitiesToAnsFrequencies(probabilities, tableSize) {
  const raw = probabilities.map((probability) => probability * tableSize);
  const frequencies = raw.map((value) => Math.max(1, Math.floor(value)));
  let difference = tableSize - frequencies.reduce((sum, frequency) => sum + frequency, 0);
  ...
  if (difference > 0) {
    ...
    while (difference > 0) {
      frequencies[order[cursor].index] += 1;
      difference -= 1;
    ...
    while (remaining > 0) {
      const targetIndex = order[cursor].index;

      if (frequencies[targetIndex] > 1) {
        frequencies[targetIndex] -= 1;
        remaining -= 1;
      }
...
function transformRandomGraphEdges(edges, originalNodes, expandedNodes, insertedNode, targetNode) {
  const transformed = new Map();
  const usableNodes = expandedNodes - 1;
  const expandedCapacity = graphEdgeCapacity(usableNodes);
  const permutationKey = reversibleGraphPermutationKey(insertedNode, targetNode, originalNodes, expandedNodes);

  edges.forEach(([left, right]) => {
    const [sourceLeft, sourceRight] = normalizeEdge(left, right);
    const sourceIndex = edgeIndexFromPair(sourceLeft, sourceRight, originalNodes);
    const transformedIndex = permuteGraphEdgeIndex(sourceIndex, expandedCapacity, permutationKey);
    const [usableLeft, usableRight] = edgePairFromIndex(transformedIndex, usableNodes);
    const transformedLeft = embedUsableNode(usableLeft, insertedNode);
    const transformedRight = embedUsableNode(usableRight, insertedNode);
    addEdgeToMap(transformed, transformedLeft, transformedRight);
  });

  if (transformed.size !== edges.length) {
    throw new Error("The reversible graph transformation produced an invalid edge mapping.");
  }

  const metadataTarget = embedUsableNode(targetNode, insertedNode);
  addEdgeToMap(transformed, insertedNode, metadataTarget);
  return Array.from(transformed.values());
}

function embedUsableNode(node, insertedNode) {
  return node >= insertedNode ? node + 1 : node;
}

function recoverUsableNode(node, insertedNode) {
  return node > insertedNode ? node - 1 : node;
}

function inverseTransformRandomGraphEdges(edges, originalNodes, expandedNodes, insertedNode, targetNode) {
  const usableNodes = expandedNodes - 1;
  const originalCapacity = graphEdgeCapacity(originalNodes);
  const expandedCapacity = graphEdgeCapacity(usableNodes);
  const permutationKey = reversibleGraphPermutationKey(insertedNode, targetNode, originalNodes, expandedNodes);
  const metadataTarget = embedUsableNode(targetNode, insertedNode);
  ...
  edges.forEach(([left, right]) => {
    if (
      (left === insertedNode && right === metadataTarget)
      || (right === insertedNode && left === metadataTarget)
    ) {
      return;
    }

    if (left === insertedNode || right === insertedNode) {
      return;
    }

    const usableLeft = recoverUsableNode(left, insertedNode);
    const usableRight = recoverUsableNode(right, insertedNode);
    const transformedIndex = edgeIndexFromPair(...normalizeEdge(usableLeft, usableRight), usableNodes);
    const sourceIndex = invertGraphEdgeIndex(transformedIndex, expandedCapacity, permutationKey);

    if (sourceIndex >= originalCapacity) {
      throw new Error("The transformed graph contains an edge outside the reversible source range.");
    }

    const [recoveredLeft, recoveredRight] = edgePairFromIndex(sourceIndex, originalNodes);
    addEdgeToMap(originalEdges, recoveredLeft, recoveredRight);
  });

  return Array.from(originalEdges.values());
}

function reversibleGraphPermutationKey(insertedNode, targetNode, originalNodes, expandedNodes) {
  let key = 2166136261;
  key = mixGraphKey(key, insertedNode + 1);
  key = mixGraphKey(key, targetNode + 1);
  key = mixGraphKey(key, originalNodes + 1);
  key = mixGraphKey(key, expandedNodes + 1);
  return key >>> 0;
}

function mixGraphKey(key, value) {
  let mixed = (key ^ value) >>> 0;
...
function permuteGraphEdgeIndex(edgeIndex, edgeCapacity, key) {
  return cycleWalkGraphEdgeIndex(edgeIndex, edgeCapacity, key, false);
}

function invertGraphEdgeIndex(edgeIndex, edgeCapacity, key) {
  return cycleWalkGraphEdgeIndex(edgeIndex, edgeCapacity, key, true);
}

function cycleWalkGraphEdgeIndex(edgeIndex, edgeCapacity, key, inverse) {
  if (edgeCapacity <= 1) {
    return 0;
  }

  const params = graphFeistelParams(edgeCapacity);
  let value = edgeIndex;

  for (let attempt = 0; attempt < 10000; attempt += 1) {
    value = graphFeistel(value, params, key, inverse);

    if (value < edgeCapacity) {
      return value;
    }
  }

  throw new Error("The reversible graph edge permutation did not converge.");
}

function graphFeistelParams(edgeCapacity) {
  const bitCount = Math.ceil(Math.log2(edgeCapacity));
  const halfBits = Math.max(1, Math.ceil(bitCount / 2));
  const base = 2 ** halfBits;
  const mask = base - 1;

  if (!Number.isSafeInteger(base * base)) {
    throw new Error("The graph is too large for the reversible browser permutation.");
  }

  return { base, mask };
}

function graphFeistel(value, params, key, inverse) {
  let left = Math.floor(value / params.base);
  let right = value % params.base;
  const rounds = 6;

  if (!inverse) {
    for (let round = 0; round < rounds; round += 1) {
      const nextLeft = right;
      const nextRight = (left ^ graphFeistelRound(right, key, round)) & params.mask;
      left = nextLeft;
      right = nextRight;
    }
  } else {
    for (let round = rounds - 1; round >= 0; round -= 1) {
      ...
      const previousLeft = (right ^ graphFeistelRound(previousRight, key, round)) & params.mask;
      left = previousLeft;
      right = previousRight;
    }
  }

  return (left * params.base) + right;
}

function graphFeistelRound(value, key, round) {
  let mixed = (value ^ key ^ Math.imul(round + 1, 2654435761)) >>> 0;
  mixed = Math.imul(mixed ^ (mixed >>> 16), 2246822519) >>> 0;
  mixed = Math.imul(mixed ^ (mixed >>> 13), 3266489917) >>> 0;
  return (mixed ^ (mixed >>> 16)) >>> 0;
}

function edgeIndexFromPair(left, right, nodeCount) {
  return ((left * ((2 * nodeCount) - left - 1)) / 2) + (right - left - 1);
}

function edgePairFromIndex(edgeIndex, nodeCount) {
  let remaining = edgeIndex;

  for (let left = 0; left < nodeCount - 1; left += 1) {
    const rowCount = nodeCount - left - 1;
...
function renderUniversalSummary({ h, transformsPerSequence, transformedLength, state, isError }) {
  const values = universalSummary.querySelectorAll("dd");
  values[0].textContent = typeof h === "number" ? formatNumber(h) : h;
  values[1].textContent = typeof transformsPerSequence === "number" ? formatNumber(transformsPerSequence) : transformsPerSequence;
  values[2].textContent = typeof transformedLength === "number" ? formatDecimal(transformedLength) : transformedLength;
  values[3].textContent = state;
  values[3].classList.toggle("error", isError);
}

function setUniversalStatus(state) {
  const values = universalSummary.querySelectorAll("dd");
  values[3].textContent = state;
  values[3].classList.remove("error");
}

function renderUniversalMetrics(metrics) {
  const values = universalMetrics.querySelectorAll("strong");
  ...
  values[0].textContent = `${formatDecimal(metrics.bestGainBits)} bits`;
  values[1].textContent = `${formatDecimal(metrics.bestGainPercent)}%`;
  values[2].textContent = metrics.bestCoder;
}

function renderUniversalRows(rows) {
  universalBody.innerHTML = rows.map((row) => `
    <tr>
      <td>${escapeHtml(row.coder)}</td>
      <td>${formatDecimal(row.originalMean)}</td>
      <td>${formatDecimal(row.sstMean)}</td>
      <td>${formatDecimal(row.gainBits)}</td>
      <td>${formatDecimal(row.gainPercent)}%</td>
      <td>${formatDecimal(row.improvedPercent)}%</td>
    </tr>
  `).join("");
}

function renderUniversalEmpty(message, isError = false) {
  universalBody.innerHTML = `
    <tr>
...
updateRandomGraphSourceMode();
updateStegoSourceMode();
const initialViewId = viewIdFromLocation();
window.history.replaceState({ viewId: initialViewId }, "", urlForView(initialViewId));
